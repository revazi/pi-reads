...
Every citation must reference one of the article's `sourceIds`. A citation should include the most precise available locator and may include a short supporting quote. Bounded source reads and searches suggest compact deterministic citation IDs plus stable locator fragments; generated articles may use those suggestions instead of inventing verbose identifiers. Stable `h_…`/`p_…` fragments, inclusive fragment ranges, one-based paragraph numbers, and exact heading labels resolve through the source index. Unknown, reversed, ambiguous, or mutually inconsistent locator fields fail closed. Exporters resolve source metadata from the source manifest and render destination-appropriate footnotes or endnotes.

Before generated content is persisted, every provided quote must occur exactly in the immutable source text and, when an indexed locator is present, within that resolved source range. Unsupported or empty quotes fail closed. Missing locators remain allowed but are explicitly counted with bounded citation-ID examples. Persisted `citation-grounding-v1` diagnostics report citation counts per source/indexed section, verified quote counts, and generated article sections with and without citation markers. Every variable diagnostic list is capped at 20 with a truncation flag; diagnostics identify uncited sections but never rewrite or fact-check prose.

A synthesis selecting 2–20 captured sources preserves explicit source order and requires a pre-persistence review. Duplicate source IDs are rejected. Every non-empty Markdown section must contain at least one registered citation marker; this structural gate is deliberately conservative and does not claim semantic fact-checking. The no-write review reports citation counts in selection order and all selected sources with zero citations. Persistence requires the review token for the exact mode/title/body/source order/citations/coverage and active provider/model/thinking/session identity; any change requires another review. Generation time is excluded from the token and recorded only on the final immutable article. See [multi-source cited synthesis](multi-source-synthesis.md).
