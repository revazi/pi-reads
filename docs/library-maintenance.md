...
The implementation rejects non-regular files, traversal/absolute paths, non-portable names, Unicode-normalized case-folded path collisions, depth above 16, more than 100,000 scanned files, individual files above 256 MiB, and tracked data above 2 GiB. It is a bounded in-memory verification workflow, not a streaming solution for arbitrarily large libraries.
