...
Model-visible success text contains only the identifiers, warnings, cursors, and artifact paths needed for the next operation; verbose manifests and paths stay in structured result details. Source reads/searches suggest deterministic compact citation IDs and stable fragment locators. The distributable skill remains the detailed workflow authority while persistent tool snippets retain one concise statement of each mandatory safety rule.
...
The active Pi model is the generator. The extension does not silently invoke a second model. `/reads` resolves a versioned structured generation template, then builds an ordered, hash-bound plan for 2–20 selected captured sources without loading their prose. `LibraryService` owns Pi-independent plan validation and the mandatory two-step multi-source persistence gate; the extension supplies active provider/model/thinking/session provenance. Every non-empty multi-source synthesis section must contain a registered citation marker, a conservative structural rule rather than semantic fact-checking. Review tokens exclude generation time but bind all draft, ordered-source, citation, coverage, template, and active generation-identity fields, so any meaningful change requires another review. Template definitions contain only allowlisted modes, section roles, coverage policy, word bounds, and citation budgets—never executable prompt prose from captures. Articles snapshot template origin/version and bounded budget warnings. See [multi-source cited synthesis](multi-source-synthesis.md) and [generation templates](generation-templates.md).
